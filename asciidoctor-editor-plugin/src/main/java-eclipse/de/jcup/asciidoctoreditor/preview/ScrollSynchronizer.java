...
public class ScrollSynchronizer {
    ...
    public void installInBrowser() {
        editor.getBrowserAccess().installScrollSyncListener(scrollSyncListener);
    }

    class ScrollSyncMouseListener extends MouseAdapter implements MouseWheelListener {
        ...
        private void updateScrollLevel() {
            if (percentage<5) {
                percentage=5;
            }
            String javascript = " document.body.style.zoom = '"+percentage+"%'";
            editor.getBrowserAccess().safeBrowserEvaluateJavascript(javascript);
        }
...
//            int count = e.count;
//            int scrollSize = Math.abs(count);
//            boolean scrollsUp = count > 0;
//            if (scrollsUp) {
//                zoomIn(scrollSize*10);
//            }else {
//                zoomOut(scrollSize*10);
//            }
